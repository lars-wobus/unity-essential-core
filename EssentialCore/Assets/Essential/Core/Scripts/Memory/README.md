...
The user can create multiple MonoBehaviours for different purposes. The user can create multiple MonoBehaviours for different purposes. Some could handle incoming events by themself, while others could forward events to their own listeners. Moreover the user is free in choosing c# delegates, actions, UnityEvents or something completely different, I haven't heard yet.
